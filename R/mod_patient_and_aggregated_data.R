...
mod_patient_and_aggregated_data_server <- function(id = character(), r, language = "EN", words = tibble::tibble()){
  moduleServer(id, function(input, output, session){
    ...
    observeEvent(r$chosen_study, {
      ...
      # r[[paste0(prefix, "_display_modules_all")]] <- display_modules
    ...
    ##########################################
      ...
      ##########################################
      # LOAD UI / LOAD UI MODULES              #
      ##########################################
      
      # observeEvent(r[[paste0(prefix, "_load_ui")]], {
      #   
      #   req(!is.na(r$chosen_study))
      #   req(r[[paste0(prefix, "_display_modules_all")]])
      #   
      #   # If this is not just a change of pivot order
      #   if (!grepl("change_pivot_order", r[[paste0(prefix, "_load_ui")]])){
      #     
      #     # Now we have a level for each module, order them by display order
      #     r[[paste0(prefix, "_display_modules")]] <- r[[paste0(prefix, "_display_modules_all")]] %>% dplyr::arrange(level, display_order)
      #     
      #     # This variable is also needed, when we delete or add a module element
      #     r[[paste0(prefix, "_load_ui_cards")]] <- Sys.time()
      #     
      #   } 
      #   
      ...
      output$study_menu <- renderUI({
        ...
        # Check if users has access only to aggregated data
        if (prefix == "patient_lvl" & r[[paste0(prefix, "_show_only_aggregated_data")]] == 1) show_message_bar(output, 1, "only_aggregated_data_authorized", "severeWarning", language)
        req((prefix == "patient_lvl" & r[[paste0(prefix, "_show_only_aggregated_data")]] != 1) | prefix == "aggregated")
        ...
        # First module shown
        first_module_shown <- r[[paste0(prefix, "_first_module_shown")]]
      ...
      observeEvent(r[[paste0(prefix, "_load_ui_cards")]], {
        ...
        # insertUI(
        #   selector = paste0("#", ns("study_cards")),
        #   where = "beforeEnd",
        #   ui = tagList(code_ui, br())
        # )
