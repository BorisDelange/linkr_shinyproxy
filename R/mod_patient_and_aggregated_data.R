...
mod_patient_and_aggregated_data_server <- function(id, r, language){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(r$chosen_study, {
      ...
      #         
      #         # Merge UIs
      #         r$result[[i]]$code_ui <- shiny.fluent::PivotItem(id = r$result[[i]]$id, headerText = r$result[[i]]$name, code_ui)
      #         
      #         # Server code
      #         r$result[[i]]$code_server <- "not_empty"
      #       }
      #       else r$result[[i]]$code_ui <- shiny.fluent::PivotItem(id = r$result[[i]]$id, headerText = r$result[[i]]$name, 
      #         shiny.fluent::Pivot(children_code_ui))
      #     }
      #   }
      # }
      # 
      ...
      # output$main <- renderUI({
       # shiny.fluent::Pivot(onLinkClick = htmlwidgets::JS("item => Shiny.setInputValue('current_tab', item.props.id)"), result_concat)
        # observeEvent(r$current_pivot_level, {
        #   output$test2 <- renderText(paste0("test2 = ", r$current_pivot_level))
          # shown_modules <- patient_lvl_modules %>% dplyr::filter(level == r$current_pivot_level)
          # pivot_items <- tagList()
          # sapply(1:nrow(shown_modules), function(i){
          #   pivot_items <<- tagList(pivot_items, shiny.fluent::PivotItem(id = shown_modules[[i, "id"]], headerText = shown_modules[[i, "name"]], "test"))
          # })
          # 
          # shiny.fluent::Pivot(
          #   onLinkClick = htmlwidgets::JS("item => Shiny.setInputValue('current_tab', item.props.id)"),
          #   pivot_items
          # )
        # })
      # })
