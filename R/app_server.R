...
app_server <- function(page_style, router, language){
  function( input, output, session ) {
    ...
    r$data_sources_data <- tibble::tribble(
    ...
    r$datamarts_data <- tibble::tribble(
    ...
    r$studies_data <- tibble::tribble(
      ~id, ~name, ~description, ~datamart_id, ~patient_lvl_data_module_family_id, ~aggregated_data_module_family_id, ~creator, ~datetime, ~deleted,
    ...
    r$patient_lvl_modules_families <- tibble::tribble(
      ~id, ~name, ~description, ~creator, ~datetime, ~deleted,
    ...
    r$aggregated_modules_families <- tibble::tribble(
      ~id, ~name, ~description, ~creator, ~datetime, ~deleted,
      #  ~`Module family ID`, ~`Module family name`, ~`Module family description`, ~`Creator`, ~`Date & time`, ~`Deleted`,
