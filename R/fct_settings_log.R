...
report_bug <- function(r = shiny::reactiveValues(), output, error_message = character(), 
  error_name = character(), category = character(), error_report = character(), i18n = R6::R6Class(), ns = character()){
  
  # print(error_report)
  
  # Notification to user
  if (error_message %not_in% c("fail_load_datamart", "fail_load_scripts")){
    
    if (length(ns) > 0) show_message_bar(output, 3, error_message, "severeWarning", i18n = i18n, ns = ns)
    if (length(ns) == 0) show_message_bar(output, 3, error_message, "severeWarning", i18n = i18n)
  }
