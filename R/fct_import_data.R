...
#' }
import_datamart <- function(r = shiny::reactiveValues(), datamart_id = integer(), data = tibble::tibble(), type = "patients", save_as_csv = TRUE, rewrite = FALSE, language = "EN"){
  ...
  if (save_as_csv & !rewrite & file.exists(path)){
    tryCatch(return(r[[type]] <- readr::read_csv(path)), 
             error = function(e) stop(translate(language, "error_loading_csv")), warning = function(w) stop(translate(language, "error_loading_csv")))
  }
  
  # Transform as tibble
  tryCatch(tibble::as_tibble(data), 
           error = function(e) stop(translate(language, "error_transforming_tibble")), warning = function(w) stop(translate(language, "error_transforming_tibble")))
  ...
  if (save_as_csv){
    ...
    if (!file.exists(path)) tryCatch(readr::write_csv(data, path),
                                     error = function(e) stop(translate(language, "error_saving_csv")), warning = function(w) stop(translate(language, "error_saving_csv")))
...
#'   
#' import_thesaurus(r = r, thesaurus_id = 5, thesaurus = thesaurus, language = "EN")
#' }
import_thesaurus <- function(r = shiny::reactiveValues(), thesaurus_id = integer(), thesaurus = tibble::tibble(), language = "EN"){
  ...
  # Check col names
  if (!identical(names(thesaurus), c("item_id", "name", "display_name", "category", "unit"))) stop(translate(language, "valid_col_names_are"), toString(var_cols %>% dplyr::pull(name)))
    
  # Check col types
  sapply(1:nrow(var_cols), function(i){
    var_name <- var_cols[[i, "name"]]
    if (var_cols[[i, "type"]] == "integer") if (!is.integer(thesaurus[[var_name]])) stop(paste0(translate(language, "column"), " ", var_name, " ", translate(language, "type_must_be_integer")))
    if (var_cols[[i, "type"]] == "character") if (!is.character(thesaurus[[var_name]])) stop(paste0(translate(language, "column"), " ", var_name, " ", translate(language, "type_must_be_character")))
  })
  ...
  tryCatch(actual_items <- 
           ...
           error = function(e) stop(translate(language, "error_get_actuel_items")), warning = function(w) stop(translate(language, "error_get_actuel_items")))
  ...
  if (nrow(items_to_insert) == 0) stop(translate(language, "thesaurus_no_items_to_insert")) 
  ...
  tryCatch(DBI::dbAppendTable(r$db, "thesaurus_items", items_to_insert),
           error = function(e) stop(translate(language, "thesaurus_error_append_table")), warning = function(w) stop(translate(language, "thesaurus_error_append_table")))
