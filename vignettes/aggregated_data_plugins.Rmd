...
temp <- function(){
    ...
    new_words <- tibble::tribble(~language, ~reference_word, ~translated_word,
        ...
        "EN", "exclusion_criteria_management", "Exclusion criterion management",
...
centered_cols <- c("id", "datetime", "display_order", "action")
column_widths = c("id" = "100px", "datetime" = "180px", "action" = "80px", "display_order" = "100px")
...
observeEvent(r$plugins_load_%group_id%_%study_id%, {
    ...
    tryCatch({
        ...
        options <- list()
        if (nrow(r$plugins_load_%group_id%_%study_id%) > 0) options <- convert_tibble_to_list(data = r$plugins_load_%group_id%_%study_id% %>% dplyr::mutate(n = 1:dplyr::n()), key_col = "n", text_col = "n")
        
        data_temp <- r$plugins_load_%group_id%_%study_id% %>% dplyr::rowwise() %>%
            dplyr::mutate(
                display_order = as.character(
                    div(
                        shiny.fluent::Dropdown.shinyInput(ns(paste0("dropdown_%group_id%_%study_id%_", id)),
                            options = options, value = display_order,
...
observeEvent(input$delete_confirmed_%group_id%_%study_id%, {
    ...
    sql <- glue::glue_sql("DELETE FROM modules_elements_options WHERE id = {row_deleted}", .con = r$db)
    query <- DBI::dbSendStatement(r$db, sql)
    DBI::dbClearResult(query)
